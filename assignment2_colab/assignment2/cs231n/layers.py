...
def affine_forward(x, w, b):
    ...
    temp = np.reshape(x,(x.shape[0],-1)) #temp (N,N*d_1...*d_k/N)----(N,d_1*...*d_k) 
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        x_variance = np.var(x, axis = 0) + eps
        std = np.sqrt(x_variance)
        x_norm = (x - x_mean) / std
        ...
        cache = (x, x_norm, gamma, x_mean, x_variance, std)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, x_norm, gamma, x_mean, x_variance, std = cache
    N, D = dout.shape
...
def batchnorm_backward_alt(dout, cache):
    ...
    N = 1.0 * x.shape[0]
    dfdu = dout * gamma
    dfdv = np.sum(dfdu * (x - x_mean) * -0.5 * x_variance ** -1.5, axis = 0)
    dfdw = np.sum(dfdu * -1 / std, axis = 0) + dfdv * np.sum(-2/N * (x - x_mean), axis = 0)
    dx = dfdu / std + dfdv * 2/N * (x - x_mean) + dfdw / N
