...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.normalization == None:      #不使用归一化
                out, cache = affine_relu_forward(x, W, b)
            elif self.normalization == "batchnorm":     # batchnorm
            ...
            if self.use_dropout:          # 使用droupout
        ...
        dscores, dw, db = affine_backward(dscores, caches[self.num_layers - 1])
        dw += self.reg * self.params['W' + str(self.num_layers)]
        grads['W' + str(self.num_layers)] = dw
        grads['b' + str(self.num_layers)] = db
        
        for i in range(self.num_layers - 2, -1, -1): # num_layer-2 开始每次1，一直到-1
          ...
          if self.normalization == None:
            ...
            dscores, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(dscores, caches[i])
            dw += self.reg * self.params['W' + str(i + 1)]
            grads['W' + str(i + 1)] = dw
            grads['b' + str(i + 1)] = db
            ...
            dscores, dw, db, dgamma, dbeta = affine_layernorm_relu_backward(dscores, caches[i])
            dw += self.reg * self.params['W' + str(i + 1)]
            grads['W' + str(i + 1)] = dw
            grads['b' + str(i + 1)] = db
            grads['gamma' + str(i + 1)] = dgamma
